...
const ChatBox = () => {
  const {selectedchats, theme, user, setUser, axios, token, setToken} = useAppContext()
  ...
  const onSubmit = async (e) => {
    try {
      e.preventDefault()
      if(!user)
        return toast.message('Login to send message')
      setLoading(true)
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2x1:pr-40'>
      ...
      <div ref={containerRef} className='flex-1 mb-5 overflow-y-auto'> 
          ...
          loading && <div className='loader flex items-center gap-1.5'>
            {
              ...
              <div className='w-1.5 h-1.5 rounded-full bg-gray-500 dark:bg-white animate-bounce'></div> */
              [...Array(3)].map((_, i) => (
                <div key={i} className='w-1.5 h-1.5 rounded-full bg-gray-500 dark:bg-white animate-bounce'></div>
              ))
            }
          </div>
        }
        ...
        <form onSubmit={onSubmit} className='bg-primary/20 items-center gap-4 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609F]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex'>
