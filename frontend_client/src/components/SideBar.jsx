...
const SideBar = ({isMenuOpen, setIsMenuOpen}) => {
  const {navigate, chats, setSelectedChats, theme, setTheme, user, createNewChat, axios, setChats, fetchUserChats, token, setToken} = useAppContext()
  ...
  const deleteChat = async (e, chatId) => {
    try {
      ...
      if(!confirm)
        return
      const {data} = await axios.post('/api/chat/deletechat', {chatId}, {headers: {Authorization:token}}) 
      if(data.success)
      {
  ...
  return (
    <div className={`flex flex-col h-full min-w-72 p-5 overflow-hidden dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {chats.length > 0 && <p className='mt-2 text-sm'>Recent Chats</p>}
      {
        chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
          <div key={chat._id} onClick={()=> {navigate('/'); setSelectedChats(chat); setIsMenuOpen(false)}} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
            <div>
